...
public class Game extends Canvas implements Runnable {
	 ...
	 */
	public enum STATE {
		Menu, Help, Help2, Help3, Game, GameOver, GameWon, Upgrade, Boss, Pause, PauseH1, PauseH2, PauseH3, PauseShop, Leaderboard, GameWonEasy, GameEasy, CharacterCustomization
	}; // Add CC here I think????
	...
	private void tick() throws IOException, JSONException {
		...
		if (gameState == STATE.Game || gameState == STATE.Boss && Spawn1to10.LEVEL_SET != 3) {// game is running
		...
		} else if (gameState == STATE.Menu || gameState == STATE.Help || gameState == STATE.Help2 || gameState == STATE.Help3 || gameState == STATE.CharacterCustomization) {// user is on menu, update the menu items
			menu.tick();
			...
			gameWon.tick();
		// I think i have to write my code here
		// } else if (gameState == STATE.CC) {
			// CC.tick();
		}
		...
		} else if (gameState == STATE.Menu || gameState == STATE.Help || gameState == STATE.Help2 || gameState == STATE.Help3 || gameState == STATE.CharacterCustomization) {
			gameMIDIPlayer.StopMidi();
			bossMIDIPlayer.StopMidi();
			upgradeMidiPlayer.StopMidi();
			ShopMIDIPlayer.StopMidi();
		...
		if (gameState == STATE.Pause || gameState == STATE.PauseH1 || gameState == STATE.PauseH2
			...
			if (gameState == STATE.Game || gameState == STATE.Boss || gameState == STATE.GameEasy) {// user is
				...
				hud.render(g);
					...
					|| gameState == STATE.Help3 || gameState == STATE.CharacterCustomization) {// user is in help or the
